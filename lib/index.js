...
const { AprilTagDetector } = require(
  join(__dirname, '..', 'build', 'Release', 'apriltag.node')
);
...
export class AprilTag {
  ...
  ensureInitialized() {
    ...
    this._initializationPromise = new Promise((resolve, reject) => {
      this.detector.initializeAsync((error, result) => {
        if (error) {
          reject(error);
        } else {
          resolve(result);
  ...
  detect(width, height, imageData) {
    ...
    if (buffer.length < width * height) {
      throw new Error(
        `Buffer too small: expected at least ${width * height} bytes, got ${buffer.length}`
      );
    }
  ...
  async detectAsync(width, height, imageData) {
    if (typeof width !== 'number' || typeof height !== 'number') {
      throw new Error('Width and height must be numbers');
    }
    ...
    return new Promise((resolve, reject) => {
      this.detector.detectAsync(width, height, buffer, (error, results) => {
        if (error) {
          reject(error);
        } else {
          resolve(results);
