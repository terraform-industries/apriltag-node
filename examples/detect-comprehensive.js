...
async function detectWithFamily(
  imageData,
  width,
  height,
  family,
  ...
  try {
    ...
    for (const family of families) {
      ...
      for (const decimate of decimateValues) {
        const result = await detectWithFamily(
          data,
          info.width,
          info.height,
          family,
        ...
        if (result.detections.length > 0) {
          ...
          console.log(
            `  ✓ Found ${result.detections.length} tags with decimate=${decimate}`
          );
        }
      }
    }
    ...
    if (allResults.length > 0) {
      console.log(`\n${'='.repeat(60)}`);
      console.log(
        `SUMMARY: Found detections with ${allResults.length} different configurations:`
      );
      ...
      allResults.forEach((result, configIndex) => {
        ...
        result.detections.forEach((detection, index) => {
          ...
          console.log(
            `    Decision margin: ${detection.decision_margin.toFixed(3)}`
          );
          console.log(
            `    Center: [${detection.center[0].toFixed(1)}, ${detection.center[1].toFixed(1)}]`
          );
          console.log(`    Corners:`);
      ...
      console.log(`\n${'='.repeat(60)}`);
      console.log(
        `BEST RESULT: ${bestResult.family} with ${bestResult.detections.length} detections`
      );
      console.log(`${'='.repeat(60)}`);
    } else {
