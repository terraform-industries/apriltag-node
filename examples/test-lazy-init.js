...
try {
  ...
  if (constructorTime > 1000) { // Constructor should be fast (< 1 second)
    console.error('✗ Constructor too slow - lazy initialization may not be working');
    process.exit(1);
  }
  ...
  console.log(`✓ First detection completed in ${firstDetectionTime.toFixed(1)}ms`);
  ...
  console.log(`✓ Second detection completed in ${secondDetectionTime.toFixed(1)}ms`);
  ...
  if (firstDetectionTime < constructorTime) {
    console.error('✗ First detection was faster than expected - initialization may not be lazy');
    process.exit(1);
  }
  
  console.log('\n🎉 Lazy initialization test passed!');
  console.log(`Constructor: ${constructorTime.toFixed(1)}ms (fast)`);
  console.log(`First detection: ${firstDetectionTime.toFixed(1)}ms (with initialization)`);
