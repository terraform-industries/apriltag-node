...
async function detectWithSettings(
  imageData,
  width,
  height,
  family,
...
async function focusedDetection(imagePath) {
  ...
  try {
    ...
    for (const config of testConfigs) {
      console.log(
        `Testing ${config.family} with decimate=${config.decimate}...`
      );
      ...
      if (detections.length > 0) {
        ...
        detections.forEach((detection, index) => {
          ...
          console.log(
            `  Decision margin: ${detection.decision_margin.toFixed(1)}`
          );
          console.log(
            `  Center: [${detection.center[0].toFixed(0)}, ${detection.center[1].toFixed(0)}]`
          );
          console.log(`  Corners:`);
          detection.corners.forEach((corner, i) => {
            console.log(
              `    [${corner[0].toFixed(0)}, ${corner[1].toFixed(0)}]`
            );
          });
          console.log('');
        });
